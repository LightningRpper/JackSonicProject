...
function collision_script_right_line(){
	mask = argument0;
	line = floor(mask*2.5);
	...
	if collision_line(x+(acos*mask)+(asin*mask),y+(asin*mask)+(acos*mask),x+(acos*mask)+(asin*line),y-(asin*mask)+(acos*line),objstompable,true,true) return true;
	return false;
}